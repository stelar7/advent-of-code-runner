...
void get_file(const char* file_name, char* data, char*& end_data, size_t size)
{
	...
	assert(len + 8 < size); // + 8 because of OOB integer reads
	end_data = data + len;
...
void eat_line(char*& input, char* end)
{
	while (input < end && *input != '\n')
		input++;
}
...
{
#if _DEBUG
	printf("You're running the debug version. Use the release version!\n\n");
#endif
