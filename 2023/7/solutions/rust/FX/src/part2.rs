...
impl TryFrom<&[Card; 5]> for Type {
    ...
    fn try_from(cards: &[Card; 5]) -> Result<Self, Self::Error> {
        fn count(cards: &[Card; 5]) -> ([i32; 13], i32) {
            let mut counts = [0; 13];
            let mut jokers = 0;
            for &card in cards {
                if card == Card::J {
        ...
        let (counts, jokers) = count(cards);

        if counts.into_iter().any(|c| c >= 5 - jokers) {
            return Ok(Self::FiveOfAKind);
        }

        if counts.into_iter().any(|c| c >= 4 - jokers) {
            return Ok(Self::FourOfAKind);
        }

        if let Some(idx) = counts.into_iter().position(|c| c >= 3 - jokers) {
            let jokers_used = 3 - counts[idx];

            if counts[..idx].iter().any(|&c| c >= 2 - jokers + jokers_used)
                || counts[idx + 1..]
                    .iter()
                    .any(|&c| c >= 2 - jokers + jokers_used)
            {
                return Ok(Self::FullHouse);
            }
        }

        if counts.into_iter().any(|c| c >= 3 - jokers) {
            return Ok(Self::ThreeOfAKind);
        }

        if let Some(idx) = counts.into_iter().position(|c| c >= 2 - jokers) {
            let jokers_used = 2 - counts[idx];
        ...
        if counts.into_iter().filter(|&c| c == 1).count() == 5 {
